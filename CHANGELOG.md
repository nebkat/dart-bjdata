...
- Add Structure-of-Arrays (SoA) support
  ...
  - `BjdataSoaLayout.columnMajor` (or `--column-major`) packs each field
    contiguously instead of each record. Both layouts cost the same, so the choice is
    about how the consumer reads the data
  ...
  - Decoding is unaffected by the layout: both layouts and a plain array of objects
    decode to the same `List` of record `Map`s, and N-dimensional containers keep their
    nesting
  - Fixed-length, dictionary and offset-table string storage, nested objects, fixed
    arrays, boolean and null fields are all supported. String columns are written as a
    dictionary, or as an offset table past 65535 distinct values; a column of single
    ASCII characters is written as `char`
- Add `BjdataConfig`, which carries the encoding settings for `bjdataEncode`,
  ...
  - `version` caps what may be written: `BjdataVersion.draft3` (or `--draft=3`) never
    ...
    byte-identical
  - `BjdataConfig.draft3` and `BjdataConfig.draft4` name a whole configuration for one
    revision, and `BjdataVersion` carries its draft number as `value`, with
    `BjdataVersion.fromValue`
