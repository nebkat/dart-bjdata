...
void main() {
  ...
  group('soa', () {
    ...
    group('layout', () {
      ...
      test('column-major opens with an object marker', () {
        ...
        // The marker chooses the payload order, not the type of the value: a
        // container opened with '{' still holds records rather than columns.
        expect(bjdataDecode(encoded), isA<List<Object?>>());
      });

      test('both layouts decode to the same records', () {
        for (final layout in [BjdataSoaLayout.rowMajor, BjdataSoaLayout.columnMajor]) {
          expect(bjdataDecode(bjdataEncode(records, config: BjdataConfig(soa: layout))), records, reason: '$layout');
        }
      });
      ...
      test('offset tables follow the payload in both layouts', () {
        ...
        for (final layout in [BjdataSoaLayout.rowMajor, BjdataSoaLayout.columnMajor]) {
          expect(bjdataDecode(bjdataEncode(table, config: BjdataConfig(soa: layout))), table, reason: '$layout');
        }
      });
      ...
      test('n-dimensional works in both layouts', () {
        ...
        for (final layout in [BjdataSoaLayout.rowMajor, BjdataSoaLayout.columnMajor]) {
          expect(bjdataDecode(bjdataEncode(grid, config: BjdataConfig(soa: layout))), grid, reason: '$layout');
        }
      });
  ...
  group('config', () {
    ...
    group('version', () {
      ...
      test('is numbered the way the specification numbers drafts', () {
        expect(BjdataVersion.draft3.value, 3);
        expect(BjdataVersion.draft4.value, 4);
        expect(BjdataVersion.values.last, BjdataVersion.draft4, reason: 'values are ordered oldest to newest');
        expect(BjdataVersion.fromValue(3), BjdataVersion.draft3);
        expect(BjdataVersion.fromValueOrNull(4), BjdataVersion.draft4);
        expect(BjdataVersion.fromValueOrNull(9), isNull);
        expect(() => BjdataVersion.fromValue(9), throwsFormatException);
      });
    ...
    test('draft4 names what the defaults already do', () {
      expect(BjdataConfig.draft4.version, BjdataVersion.draft4);
      expect(BjdataConfig.draft4, const BjdataConfig());
      expect(bjdataEncode(records, config: BjdataConfig.draft4).hex, bjdataEncode(records).hex);
      expect(BjdataConfig.draft4, isNot(BjdataConfig.draft3));
    });
