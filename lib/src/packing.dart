...
/// The number of bytes [string] occupies as UTF-8, without encoding it.
///
/// Unpaired surrogates cost three bytes, which is what `utf8.encode` spends on
/// the replacement character it writes for one.
int bjdataUtf8ByteLength(String string) {
  var length = 0;
  for (var i = 0; i < string.length; i++) {
    ...
    if (unit < 0x80) {
      ...
      length += 2;
        ...
        unit < 0xDC00 &&
        i + 1 < string.length &&
        (string.codeUnitAt(i + 1) & 0xFC00) == 0xDC00) {
