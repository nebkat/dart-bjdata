...
class BjdataReader {
  ...
  ({BjdataMarker? strongType, BjdataSoaSchema? soaSchema, int? count, List<int>? dimensions, bool columnMajor})
    ...
    // Read strong type, or the SoA schema a '{' introduces in its place
    BjdataMarker? strongType;
    BjdataSoaSchema? soaSchema;
    final offsetBeforeType = _offset;
    switch (_readMarker()) {
      case BjdataMarker.objectOpen:
        soaSchema = BjdataSoaSchema(_readSoaFields());
      case final marker when marker.isValidStrongType:
        strongType = marker;
      case final marker:
        throw FormatException('Invalid strong type: $marker', _bytes, offsetBeforeType);
    }
    ...
    // Read count
    final offsetBeforeCount = _offset;
    final (:count, :dimensions, :columnMajor) = _readCount();
  ...
  /// Reads an object, with the leading `{` already consumed.
  ///
  /// A `{` also opens a column-major SoA container, which holds records rather
  /// than named fields of its own, so this returns a [List] for one of those.
  Object? _readMap() {
    final offsetBefore = _offset;
    final (:strongType, :soaSchema, :count, :dimensions, columnMajor: _) = _readStrongTypeAndCount();

    if (soaSchema != null) {
      return _readSoa(soaSchema, dimensions ?? [count!], columnMajor: true);
    }
  ...
  ///
  /// [columnMajor] only changes the order the payload is walked in: `{$` takes one
  /// field across every record before the next, `[$` one record at a time. Either
  /// way the records are the same, nested according to [dimensions], so a `4x3`
  /// grid decodes to a list of four lists of three records.
  List<Object?> _readSoa(BjdataSoaSchema schema, List<int> dimensions, {required bool columnMajor}) {
