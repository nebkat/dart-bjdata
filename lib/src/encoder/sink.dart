...
abstract class _BjdataWriter<T> {
  ...
  void writeStringWithoutMarker(String string) {
    final bytes = _utf8Bytes(string);
    writeInt(bytes.length);
    writeStringContents(string, bytes);
  }
  ...
  /// Append a string contents to the BJData output.
  ///
  /// [bytes] is [string] encoded as UTF-8, which the caller has already needed
  /// for the byte length it writes ahead of the contents.
  void writeStringContents(String string, Uint8List bytes);
  ...
  void writeSoaField(
    ...
    switch (type) {
      ...
      case BjdataSoaDictionaryType(:final indices, :final huge, :final indexMarker):
        if (huge ? value is! BigInt : value is! String) {
          throw _soaTypeError(name, value, huge ? 'a BigInt' : 'a String');
        }
        final index = indices[value!];
        if (index == null) {
      ...
      case BjdataSoaOffsetType(:final offsetMarker):
        if (value is! String) throw _soaTypeError(name, value, 'a String');
        writeIntWithoutMarker(offsetMarker, offsets[path]!.slotOf(value));
      case BjdataSoaObjectType(:final fields):
  ...
  void writeFixedStringContents(String string, int byteLength) {
    ...
    writeStringContents(string, bytes);
    writePadding(byteLength - bytes.length);
  }
  ...
  void writeSoaOffsetTables(Map<String, _SoaOffsetWriter> offsets) {
    for (final writer in offsets.values) {
      ...
      final encoded = [for (final value in writer.values) _utf8Bytes(value)];
      newLine();
      var offset = 0;
      ...
      // The table holds one entry per record plus one, so the slots left over
      // by repeated values are empty runs at the end of the buffer.
      for (var slot = writer.values.length; slot < writer.records; slot++) {
        writeIntWithoutMarker(marker, offset);
      }
      newLine();
      for (var i = 0; i < encoded.length; i++) {
        writeStringContents(writer.values[i], encoded[i]);
      }
    }
  }
}
...
///
/// `utf8.encode` returns a plain `List<int>` before Dart 3.2, so the cast earns
/// its keep as long as the package supports 3.1.
// ignore: unnecessary_cast (https://github.com/dart-lang/sdk/issues/52801)
Uint8List _utf8Bytes(String string) => utf8.encode(string) as Uint8List;
...
class BjdataBufferWriter extends _BjdataWriter {
  ...
  @override
  void writeStringContents(String string, Uint8List bytes) => writeBytes(bytes);
...
class BjdataBlockNotationStringifier extends _BjdataWriter {
  ...
  @override
  void writeStringContents(String string, Uint8List bytes) => writeBlock(string);
...
///
/// Equal values share a slot, so the buffer holds each distinct string once. The
/// specification indexes the offset table by the value stored in each record, so
/// nothing requires a record's slot to be its own position.
class _SoaOffsetWriter {
  _SoaOffsetWriter(this.type);

  final BjdataSoaOffsetType type;

  /// The distinct values, in the order they are written to the buffer.
  final List<String> values = [];

  /// The slot each distinct value occupies.
  final Map<String, int> _slots = {};

  /// How many records have been written, which is one fewer than the number of
  /// offsets the table holds.
  int records = 0;

  /// The slot of [value], adding it to the buffer if it is new.
  int slotOf(String value) {
    records++;
    return _slots.putIfAbsent(value, () {
      values.add(value);
